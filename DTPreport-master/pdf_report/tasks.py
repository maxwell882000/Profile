...
def make_pdf(id):
    ...
    filename = "main_{}_{}.pdf".format(new_report_pdf.report_id, datetime.datetime.now().timestamp())
...
def make_pdf_additional(id):
    ...
    new_report_pdf = Report.objects.get(report_id=id)
    data = generate_pdf_report(id, obj=obj)
    filename = "second_{}_{}.pdf".format(new_report_pdf.report_id, datetime.datetime.now().timestamp())
...
def make_pdf_enumeration(id):
    ...
    filename = "main_enumeration_{}__{}.pdf".format(new_report_pdf.report_id, datetime.datetime.now().timestamp())
