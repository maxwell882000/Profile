...
@section("content")
    ...
    <div class="flex flex-col space-y-1">
        ...
        <div class="flex flex-row justify-between pr-10">
            <x-helper.text.title>{{__("Цвета") ?? ""}}</x-helper.text.title>
            <div class='self-end'>
                    <x-helper.button.main_button href='{{route("admin.color.create", array_merge([], \Request::query())) ?? ""}}'>
                              {{__('Создать')}}
...
<livewire:admin.pages.color.color-index />
        </div>
    </div>
@endsection
