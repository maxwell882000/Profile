...
@section("content")
    ...
    <div class="flex flex-col space-y-1">
        ...
        <div class="flex flex-row justify-between pr-10">
            <x-helper.text.title>{{__("Политика и Конфеденциальность") ?? ""}}</x-helper.text.title>
            <div class='self-end'>
                    <x-helper.button.main_button href='{{route("admin.policy.create", array_merge([], \Request::query())) ?? ""}}'>
                              {{__('Создать')}}
...
<livewire:admin.pages.policy-index.policy-index />
        </div>
    </div>
@endsection
