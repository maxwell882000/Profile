...
<livewire:components.drop-down.drop-down-search
            ...
            Введите название
                 ...
                 <x-helper.input.input name='title[ru]' class_div='w-1/4' label='{{__("на русском языке")}}'  value='{{old("title") ? old("title")["ru"] ?? $entity->title["ru"] ?? " " : $entity->title["ru"] ?? " "}}'  /><x-helper.input.input name='title[uz]' class_div='w-1/4' label='{{__("o`zbek tilda")}}'  value='{{old("title") ? old("title")["uz"] ?? $entity->title["uz"] ?? " " : $entity->title["uz"] ?? " "}}'  /><x-helper.input.input name='title[en]' class_div='w-1/4' label='{{__("in english")}}'  value='{{old("title") ? old("title")["en"] ?? $entity->title["en"] ?? " " : $entity->title["en"] ?? " "}}'  />
             </div>
            </div>
            
<x-helper.input.input name='number' type='number'
            label='{{__("Введите количество данного товара")}}' value='{{old("number") ?? $entity->number ?? " "}}' id='number' oninput='' placeholder=''  onkeyup=""  />
...
<x-helper.input.input name='price' type='number'
            label='{{__("Введите цену")}}' value='{{old("price") ?? $entity->price ?? " "}}' id='price' oninput='' placeholder=''  onkeyup=""  />
<x-helper.input.input name='percentage' type='number'
            label='{{__("Укажите скидку (%)")}}' value='{{old("percentage") ?? $entity->percentage ?? " "}}' id='percentage' oninput='' placeholder=''  onkeyup=""  />
<x-helper.input.input name='weight' type='number'
            label='{{__("Введите вес(кг)")}}' value='{{old("weight") ?? $entity->weight ?? " "}}' id='weight' oninput='' placeholder=''  onkeyup=""  />
<x-helper.input.input_checked name='productDay->checked' type='checkbox'
            label='{{__("Продукт дня")}}' value='{{old("productDay->checked") ?? $entity->productDay->checked ?? " "}}' id='productDay->checked' oninput='' placeholder=''  onchange=""  />
<x-helper.input.input_checked name='productHit->checked' type='checkbox'
            label='{{__("Хит продаж")}}' value='{{old("productHit->checked") ?? $entity->productHit->checked ?? " "}}' id='productHit->checked' oninput='' placeholder=''  onchange=""  />
...
<x-helper.container.container :title='__("Большое описание")' 	class='space-y-4 flex flex-wrap justify-between'>
            ...
            Введите заголовок
                 ...
                 <div class=' flex flex-row space-x-2'>
                 <x-helper.input.input name='description->header[ru]' class_div='w-1/4' label='{{__("на русском языке")}}'  value='{{old("description->header") ? old("description->header")["ru"] ?? $entity->description->header["ru"] ?? " " : $entity->description->header["ru"] ?? " "}}'  /><x-helper.input.input name='description->header[uz]' class_div='w-1/4' label='{{__("o`zbek tilda")}}'  value='{{old("description->header") ? old("description->header")["uz"] ?? $entity->description->header["uz"] ?? " " : $entity->description->header["uz"] ?? " "}}'  /><x-helper.input.input name='description->header[en]' class_div='w-1/4' label='{{__("in english")}}'  value='{{old("description->header") ? old("description->header")["en"] ?? $entity->description->header["en"] ?? " " : $entity->description->header["en"] ?? " "}}'  />
             </div>
            </div>
