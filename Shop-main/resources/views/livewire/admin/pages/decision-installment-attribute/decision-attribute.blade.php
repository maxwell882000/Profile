...
<x-helper.button.base_button 	class='bg-blue-600 hover:bg-blue-400 '	@click='show = false '	wire:click='acceptInstallment'>Да</x-helper.button.base_button>
            ...
            <div 	x-data='{ show: false, reason:{}, hold_money: false }'>
...
<span 	class='text-lg  font-medium'>{{__('Укажите причину вашего отказа')}}</span>			
            ...
            Причина
                 ...
                 <x-helper.input.input name='reason[ru]' class_div='w-1/4' label='{{__("на русском языке")}}'  value='{{old("reason") ? old("reason")["ru"] ?? $entity->reason["ru"] ?? " " : $entity->reason["ru"] ?? " "}}' x-model=reason.ru /><x-helper.input.input name='reason[uz]' class_div='w-1/4' label='{{__("o`zbek tilda")}}'  value='{{old("reason") ? old("reason")["uz"] ?? $entity->reason["uz"] ?? " " : $entity->reason["uz"] ?? " "}}' x-model=reason.uz /><x-helper.input.input name='reason[en]' class_div='w-1/4' label='{{__("in english")}}'  value='{{old("reason") ? old("reason")["en"] ?? $entity->reason["en"] ?? " " : $entity->reason["en"] ?? " "}}' x-model=reason.en />
             </div>
            </div>
...
<span >{{__('Хотите снять деньги ?')}}</span>			
<input 	type='checkbox'	x-model='hold_money'>
            </div>			
...
<x-helper.button.base_button 	class='bg-red-600 hover:bg-red-400 '	@click='$wire.set(`reason`,reason); show = false; $wire.denyInstallment(hold_money)'>Отказать</x-helper.button.base_button>
            </div>
            </div>
            </div>
        </div>
