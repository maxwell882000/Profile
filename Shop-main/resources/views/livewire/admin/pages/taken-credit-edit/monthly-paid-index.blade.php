...
<x-helper.input.input name='phone' type='number'
            label='{{__("Введите номер телефона")}}' value='{{old("phone") ?? $entity->phone ?? " "}}' id='phone' oninput='' placeholder=''  onkeyup=""  	wire:model.defer='phone'/>			

            <div 	class='pt-1 justify-end flex flex-row  space-x-2'>
                			

            <div 	class='self-end'>
                			
<x-helper.button.base_button 	class='bg-gray-600 hover:bg-gray-400 '	@click='show = false'>Назад</x-helper.button.base_button>			
<x-helper.button.base_button 	class='bg-blue-600 hover:bg-blue-400 '	@click='show = false'	wire:click='sendNotPaid'>Отправить</x-helper.button.base_button>
            </div>
            </div>
            </div>
        </div>
